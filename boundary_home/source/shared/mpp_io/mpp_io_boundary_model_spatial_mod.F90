...
!  each
   ...
   integer ::     umdt_varid,     vmdt_varid   
   integer ::     dudx_varid,     dudy_varid
   integer ::     dvdx_varid,     dvdy_varid
   integer ::     dudz_varid,     dvdz_varid
   ...
   integer :: vtur_u2d_varid,     vtur_v2d_varid
   integer ::  fric_2d_varid,     cd_2d_varid
   integer :: rain_acc_varid
  
contains
  
subroutine write_boundary_model_spatial ( um, vm, umdt, vmdt, dudx, dudy, dudz, dvdx, dvdy, dvdz, pgfx, pgfy, wm, tur, &
                        znot, mask, rain_acc, recs, nx, ny, nz, ngx, ngy, npx, npy, vtur_u2d, vtur_v2d, fric_2d, cd_2d )
...
!       if ( TRIM(diag_table_list(i)) == 'vtur_u2d') then
!         retval = nf_def_var(ncid, 'vtur_u2d'  , NF_REAL, ndims_3d, dimids_3d, vtur_u2d_varid)
!         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_def_var/vtur_u2d_varid' )
!       endif
!     
!       if ( TRIM(diag_table_list(i)) == 'vtur_v2d') then
!         retval = nf_def_var(ncid, 'vtur_v2d'  , NF_REAL, ndims_3d, dimids_3d, vtur_v2d_varid)
!         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_def_var/vtur_v2d_varid' )
!       endif

       if ( TRIM(diag_table_list(i)) == 'fric_2d') then
         retval = nf_def_var(ncid, 'fric_2d'  , NF_REAL, ndims_3d, dimids_3d, fric_2d_varid)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_def_var/fric_2d_varid' )
       endif

       if ( TRIM(diag_table_list(i)) == 'cd_2d') then
         retval = nf_def_var(ncid, 'cd_2d'  , NF_REAL, ndims_3d, dimids_3d, cd_2d_varid)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_def_var/cd_2d_varid' )
       endif
       ...
       if ( TRIM(diag_table_list(i)) == 'umdt') then
         retval = nf_def_var(ncid, 'umdt'  , NF_REAL, ndims_4d, dimids_4d, umdt_varid)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_def_var/umdt_varid' )
       endif

       if ( TRIM(diag_table_list(i)) == 'vmdt') then
         retval = nf_def_var(ncid, 'vmdt'  , NF_REAL, ndims_4d, dimids_4d, vmdt_varid)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_def_var/vmdt_varid' )
       endif
       
       if ( TRIM(diag_table_list(i)) == 'vtur_u2d') then
         retval = nf_def_var(ncid, 'vtur_u2d'  , NF_REAL, ndims_4d, dimids_4d, vtur_u2d_varid)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_def_var/vtur_u2d_varid' )
       endif

       if ( TRIM(diag_table_list(i)) == 'vtur_v2d') then
         retval = nf_def_var(ncid, 'vtur_v2d'  , NF_REAL, ndims_4d, dimids_4d, vtur_v2d_varid)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_def_var/vtur_v2d_varid' )
       endif
       

       if ( TRIM(diag_table_list(i)) == 'dudx') then
         retval = nf_def_var(ncid, 'dudx'  , NF_REAL, ndims_4d, dimids_4d, dudx_varid)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_def_var/dudx_varid' )
       endif

       if ( TRIM(diag_table_list(i)) == 'dudy') then
         retval = nf_def_var(ncid, 'dudy'  , NF_REAL, ndims_4d, dimids_4d, dudy_varid)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_def_var/dudy_varid' )
       endif

       if ( TRIM(diag_table_list(i)) == 'dvdx') then
         retval = nf_def_var(ncid, 'dvdx'  , NF_REAL, ndims_4d, dimids_4d, dvdx_varid)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_def_var/dvdx_varid' )
       endif

       if ( TRIM(diag_table_list(i)) == 'dvdy') then
         retval = nf_def_var(ncid, 'dvdy'  , NF_REAL, ndims_4d, dimids_4d, dvdy_varid)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_def_var/dvdy_varid' )
       endif

       if ( TRIM(diag_table_list(i)) == 'dudz') then
         retval = nf_def_var(ncid, 'dudz'  , NF_REAL, ndims_4d, dimids_4d, dudz_varid)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_def_var/dudz_varid' )
       endif

       if ( TRIM(diag_table_list(i)) == 'dvdz') then
         retval = nf_def_var(ncid, 'dvdz'  , NF_REAL, ndims_4d, dimids_4d, dvdz_varid)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_def_var/dvdz_varid' )
       endif
       ...
       if ( TRIM(diag_table_list(i)) == 'vtur_u2d') then
         retval = nf_put_att_text(ncid, vtur_u2d_varid, UNITS, len(var_units), var_units)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_att_text/var_units' )
       endif

       if ( TRIM(diag_table_list(i)) == 'vtur_v2d') then
         retval = nf_put_att_text(ncid, vtur_v2d_varid, UNITS, len(var_units), var_units)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_att_text/var_units' )
       endif


       if ( TRIM(diag_table_list(i)) == 'fric_2d') then
         retval = nf_put_att_text(ncid, fric_2d_varid, UNITS, len(var_units), var_units)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_att_text/var_units' )
       endif

       if ( TRIM(diag_table_list(i)) == 'cd_2d') then
         retval = nf_put_att_text(ncid, cd_2d_varid, UNITS, len(var_units), var_units)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_att_text/var_units' )
       endif
       ...
       if ( TRIM(diag_table_list(i)) == 'umdt') then
         retval = nf_put_att_text(ncid, umdt_varid, UNITS, len(var_units), var_units)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_att_text/var_units' )
       endif

       if ( TRIM(diag_table_list(i)) == 'vmdt') then
         retval = nf_put_att_text(ncid, vmdt_varid, UNITS, len(var_units), var_units)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_att_text/var_units' )
      endif

      
       if ( TRIM(diag_table_list(i)) == 'dudx') then
         retval = nf_put_att_text(ncid, dudx_varid, UNITS, len(var_units), var_units)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_att_text/var_units' )
       endif

       if ( TRIM(diag_table_list(i)) == 'dudy') then
         retval = nf_put_att_text(ncid, dudy_varid, UNITS, len(var_units), var_units)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_att_text/var_units' )
      endif

       if ( TRIM(diag_table_list(i)) == 'dvdx') then
         retval = nf_put_att_text(ncid, dvdx_varid, UNITS, len(var_units), var_units)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_att_text/var_units' )
       endif

       if ( TRIM(diag_table_list(i)) == 'dvdy') then
         retval = nf_put_att_text(ncid, dvdy_varid, UNITS, len(var_units), var_units)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_att_text/var_units' )
      endif
      
       if ( TRIM(diag_table_list(i)) == 'dudz') then
         retval = nf_put_att_text(ncid, dudz_varid, UNITS, len(var_units), var_units)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_att_text/var_units' )
       endif

       if ( TRIM(diag_table_list(i)) == 'dvdz') then
         retval = nf_put_att_text(ncid, dvdz_varid, UNITS, len(var_units), var_units)
         if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_att_text/var_units' )
      endif
...
!     if ( TRIM(diag_table_list(i)) == 'vtur_u2d') then
!       var_write_3d_r4(:,:) = vtur_u2d(:,:)
!       retval = nf_put_vara_REAL(ncid, vtur_u2d_varid, start_3d, count_3d, var_write_3d_r4)
!       if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_var_REAL/vtur_u2d' )
!    endif
!
!     if ( TRIM(diag_table_list(i)) == 'vtur_v2d') then
!       var_write_3d_r4(:,:) = vtur_v2d(:,:)
!       retval = nf_put_vara_REAL(ncid, vtur_v2d_varid, start_3d, count_3d, var_write_3d_r4)
!       if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_var_REAL/vtur_v2d' )
!    endif

     if ( TRIM(diag_table_list(i)) == 'fric_2d') then
       var_write_3d_r4(:,:) = fric_2d(:,:)
       retval = nf_put_vara_REAL(ncid, fric_2d_varid, start_3d, count_3d, var_write_3d_r4)
       if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_var_REAL/fric_2d' )
    endif

     if ( TRIM(diag_table_list(i)) == 'cd_2d') then
       var_write_3d_r4(:,:) = cd_2d(:,:)
       retval = nf_put_vara_REAL(ncid, cd_2d_varid, start_3d, count_3d, var_write_3d_r4)
       if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_var_REAL/cd_2d' )
    endif
     ...
     if ( TRIM(diag_table_list(i)) == 'umdt') then
       var_write_4d_r4(:,:,:) = umdt(:,:,:)
       retval = nf_put_vara_REAL(ncid, umdt_varid, start_4d, count_4d, var_write_4d_r4)
       if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_var_REAL/umdt' )
    endif

     if ( TRIM(diag_table_list(i)) == 'vmdt') then
       var_write_4d_r4(:,:,:) = vmdt(:,:,:)
       retval = nf_put_vara_REAL(ncid, vmdt_varid, start_4d, count_4d, var_write_4d_r4)
       if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_var_REAL/vmdt' )
    endif
    
    
     if ( TRIM(diag_table_list(i)) == 'vtur_v2d') then
       var_write_4d_r4(:,:,:) = vtur_v2d(:,:,:)
       retval = nf_put_vara_REAL(ncid, vtur_v2d_varid, start_4d, count_4d, var_write_4d_r4)
       if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_var_REAL/vtur_v2d' )
    endif

     if ( TRIM(diag_table_list(i)) == 'vtur_u2d') then
       var_write_4d_r4(:,:,:) = vtur_u2d(:,:,:)
       retval = nf_put_vara_REAL(ncid, vtur_u2d_varid, start_4d, count_4d, var_write_4d_r4)
       if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_var_REAL/vtur_u2d' )
    endif
     ...
     if ( TRIM(diag_table_list(i)) == 'dudx') then
       var_write_4d_r4(:,:,:) = dudx(:,:,:)
       retval = nf_put_vara_REAL(ncid, dudx_varid, start_4d, count_4d, var_write_4d_r4)
       if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_var_REAL/dudx' )
     endif

     if ( TRIM(diag_table_list(i)) == 'dudy') then
       var_write_4d_r4(:,:,:) = dudy(:,:,:)
       retval = nf_put_vara_REAL(ncid, dudy_varid, start_4d, count_4d, var_write_4d_r4)
       if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_var_REAL/dudy' )
     endif

     if ( TRIM(diag_table_list(i)) == 'dvdx') then
       var_write_4d_r4(:,:,:) = dvdx(:,:,:)
       retval = nf_put_vara_REAL(ncid, dvdx_varid, start_4d, count_4d, var_write_4d_r4)
       if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_var_REAL/dvdx' )
     endif

     if ( TRIM(diag_table_list(i)) == 'dvdy') then
       var_write_4d_r4(:,:,:) = dvdy(:,:,:)
       retval = nf_put_vara_REAL(ncid, dvdy_varid, start_4d, count_4d, var_write_4d_r4)
       if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_var_REAL/dvdy' )
     endif     

     if ( TRIM(diag_table_list(i)) == 'dudz') then
       var_write_4d_r4(:,:,:) = dudz(:,:,:)
       retval = nf_put_vara_REAL(ncid, dudz_varid, start_4d, count_4d, var_write_4d_r4)
       if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_var_REAL/dudz' )
     endif

     if ( TRIM(diag_table_list(i)) == 'dvdz') then
       var_write_4d_r4(:,:,:) = dvdz(:,:,:)
       retval = nf_put_vara_REAL(ncid, dvdz_varid, start_4d, count_4d, var_write_4d_r4)
       if (retval .NE. 0) call error_handler ( 'ERROR', 'write_boundary_model_spatial/nf_put_var_REAL/dvdz' )
     endif
