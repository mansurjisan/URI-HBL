...
subroutine storm_stats_named (xgrid, ygrid, time, ubot, vbot, pres_sea, xcen, ycen, xcor, ycor, &
   ...
   real(KIND=r4),                   intent(out) :: utx, uty, n
   ...
   integer         :: garb(5) ! extra columns in the track file we don't need 
   integer         :: rd1(4)  ! rd1 radius of 18m/s wind in the four quadrants
   integer         :: rd2(4)  ! rd2 radius of 26m/s wind in the four quadrants
   ...
   real(KIND=r4), dimension(im,jm) :: radial 
   real(KIND=r4), dimension(im,jm) :: tangential
...
!     print *, pres1
   ...
   ws18   = 17.49_r4*wind_scale
   ws26   = 25.72_r4*wind_scale
...
! if (uts.gt.10 .and. uts.lt. 15) then
!
!   utx  = utx*.88
!   uty  = uty*.88
!
! elseif (uts.gt.15 .and. uts.lt. 26) then  
!
!   utx  = utx*.25
!   uty  = uty*.25
!
...
!   b = wsmax**2*e*rhoair/delp
!   if ( b .gt. 3.0_r4 )  b = 3.0_r4
!   a7 = rmax**b
...
!        print *, minval(wndx), maxval(wndx)
!        print *, minval(wndx), maxval(wndy)

!        print *, "Shape of array: ", shape(wndx)
