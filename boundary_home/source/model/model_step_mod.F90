...
subroutine model_step ( model_time) 
   ...
   real(KIND=r8), dimension(nx,ny,nz) :: umdt
   real(KIND=r8), dimension(nx,ny,nz) :: vmdt   
   real(KIND=r8), dimension(nx,ny,nz) :: tur
   real(KIND=r8), dimension(nx,ny,nz) :: umvm

   real(KIND=r8), dimension(nx,ny,nz) :: dudx
   real(KIND=r8), dimension(nx,ny,nz) :: dudy
   real(KIND=r8), dimension(nx,ny,nz) :: dudz
   real(KIND=r8), dimension(nx,ny,nz) :: dvdx
   real(KIND=r8), dimension(nx,ny,nz) :: dvdy
   real(KIND=r8), dimension(nx,ny,nz) :: dvdz
   ...
   real(KIND=r8), dimension(nx,ny,nz) :: vtur_u2d
   real(KIND=r8), dimension(nx,ny,nz) :: vtur_v2d
   real(KIND=r8), dimension(nx,ny,nz) :: um0
   real(KIND=r8), dimension(nx,ny,nz) :: vm0
   real(KIND=r8), dimension(nx,ny) :: fric_2d
   real(KIND=r8), dimension(nx,ny) :: cd_2d
   real(KIND=r8), dimension(nx,ny) :: fric
   real(KIND=r8), dimension(nx,ny) :: ustar
   real(KIND=r8), dimension(nx,ny) :: uvbot
   ...
   real(KIND=r8) :: vref, znot, zkap, pmix, pmixsq
...
!  compute znot
   do j=2,ny-1
     do i=2,nx-1
       ...
       fric(i,j)     = cd(i,j)*vref**2
       ustar(i,j)    = sqrt(fric(i,j))
       uvbot(i,j)    = ustar(i,j)/0.4*log(10./znot)
       fric_y(i,j)   = fric(i,j)*vm(i,j,2)/vref
       fric_x(i,j)   = fric(i,j)*um(i,j,2)/vref
!       vtur_v2d(i,j) = fric_y(i,j)
!       vtur_u2d(i,j) = fric_x(i,j)
       cd_2d(i,j)    = cd(i,j)
       fric_2d(i,j)  = fric(i,j)
       vm(i,j,1)     = uvbot(i,j)*vm(i,j,2)/vref
       um(i,j,1)     = uvbot(i,j)*um(i,j,2)/vref
       tur_sl(i,j,1) = 0.4_r8*10.0_r8*ustar(i,j)
       tur_sl(i,j,2) = 0.4*stepz*ustar(i,j)
       znot2d(i,j)   = znot
     enddo
   enddo
...
!        intergrate vm
         ...
         vmdt(i,j,k)  = (vm(i,j,k) - vm0(i,j,k))/delt
   ...
   call update_domain ( vmdt, nx, ny, nz, npx, npy )
   call update_domain ( dvdx, nx, ny, nz, npx, npy )
   call update_domain ( dvdy, nx, ny, nz, npx, npy )
   call update_domain ( dvdz, nx, ny, nz, npx, npy )
   call update_domain ( vtur_v2d, nx, ny, nz, npx, npy )
...
!        intergrate um
         ...
         umdt(i,j,k)  = (um(i,j,k) - um0(i,j,k))/delt
      enddo
     enddo
   enddo
...
!     
   ...
   call update_domain ( umdt, nx, ny, nz, npx, npy )
   call update_domain ( dudx, nx, ny, nz, npx, npy )
   call update_domain ( dudy, nx, ny, nz, npx, npy )
   call update_domain ( dudz, nx, ny, nz, npx, npy ) 
   call update_domain ( vtur_u2d, nx, ny, nz, npx, npy )
   call update_domain ( cd_2d, nx, ny, npx, npy )
   call update_domain ( fric_2d, nx, ny, npx, npy )
...
1000 format( 2x, ' MODEL DIAGNOSTICS WRITTEN AT TIME = ', e13.6, /, &
       ...
       call write_boundary_model ( um, vm, umdt, vmdt, dudx, dudy, dudz, dvdx, dvdy, dvdz, pgfx(:,:,rloc), pgfy(:,:,rloc), wm, tur, &
